...
const handleNextStep = (params = {}) => {
  ...
  if (params.maxRounds) {
    ...
    addLog('Using default simulation rounds')
  }
  ...
  // Pass custom rounds via query if present
  if (params.maxRounds) {
 ...
 * When user returns from Step 3 to Step 2, assume exit simulation by default
 */
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          // If graceful fail, try force stop
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`Error closing environment: ${closeErr.message}`)
        // If graceful error, try force stop
        await forceStopSimulation()
      }
    } else {
      // Env not running, but process might still be there, check simulation status
...
const forceStopSimulation = async () => {
  try {
    ...
    addLog(`Exception stopping simulation: ${err.message}`)
  }
}
...
const loadSimulationData = async () => {
  try {
    ...
    addLog(`Exception loading data: ${err.message}`)
  }
}
...
onMounted(async () => {
  ...
  // Check and stop running simulation (when returning from Step 3)

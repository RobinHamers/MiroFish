            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generated, but the full section may not be complete yet) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll position to keep the button at the same location
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract currently active facts - extract all, no limit
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract interview count (e.g. "5 / 9")
    ...
    const reasonMatch = text.match(/### \[Selection Reasons for Interviewees\]\n([\s\S]*?)(?=\n---\n|\n### \[Interview Transcripts\])/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        } else if (currentName && line.trim() && !line.match(/^Not selected|^Summary|^Final choice/)) {
          // Continuation of the reason (excluding closing summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If the first question is prefixed with "1.", special handling needed
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          ...
          }, expandedEntities.value ? `Collapse ▲` : `Show all ${props.result.entities.length} entities ▼`)
        ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // If no numbers found or only one, return entire text
      if (matches.length <= 1) {

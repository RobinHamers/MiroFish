    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content is still being processed, try refreshing the graph later</span>
...
const renderGraph = () => {
  ...
  const getLinkPath = (d) => {
    ...
    // Offset perpendicular to the connection direction
  ...
  const getLinkMidpoint = (d) => {
    ...
    // Formula B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, t=0.5
  ...
  // Save references for external control
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position only, don't restart simulation (to distinguish click vs drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect actual drag start (movement past threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // First drag detection, restart simulation
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Allow simulation to cool down only if actually dragged
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Update link labels (no rotation, horizontal for clarity)

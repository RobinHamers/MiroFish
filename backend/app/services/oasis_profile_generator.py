...
1. Calls Zep retrieval functionality to enrich node information.
...
class OasisProfileGenerator:
    ...
    # Individual entity types (need specific personas)
    INDIVIDUAL_ENTITY_TYPES = [
        "student", "alumni", "professor", "person", "publicfigure", 
        "expert", "faculty", "official", "journalist", "activist"
    ]
    
    # Group/Organization entity types (need representative personas)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep doesn't have a built-in hybrid search interface, so we search edges and nodes separately and merge results.
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        # 3. Add associated nodes' detailed information
    ...
    def _generate_profile_with_llm(
        ...
        - Individual entity: Generates specific personal settings
        - Group/Institutional entity: Generates representative account settings
        """
        ...
        for attempt in range(max_attempts):
            try:
                ...
                try:
                    ...
                    logger.warning(f"JSON parse failure (attempt {attempt+1}): {str(je)[:80]}")
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in generating social media user personas. Generate detailed, realistic personas for public opinion simulation, reflecting reality as much as possible. You must return a valid JSON format, and all string values must not contain unescaped newlines. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, reflecting reality as much as possible.
...
2. persona: Detailed persona description (2000 words plain text), including:
   ...
   - Background (important experiences, connection to events, social relations)
   ...
   - Stance/Views (attitude towards topics, content that might provoke/move them)
   - Unique Traits (catchphrases, special experiences, personal hobbies)
   - Personal Memory (important part of persona, introduce this individual's connection with the event, and their existing actions and reactions in the event)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account setting for the institutional/group entity, reflecting reality as much as possible.
...
2. persona: Detailed account setting description (2000 words plain text), including:
   - Institution Basic Info (official name, nature of institution, background of establishment, main functions)
   ...
   - Institutional Memory (important part of institutional persona, introduce this institution's connection with the event, and its existing actions and reactions in the event)
3. age: Fixed to 30 (virtual age of institutional account)
4. gender: Fixed to "other" (institutional account uses 'other' for non-person)
...
Important:
- All field values must be strings or numbers, do not use null values
- persona must be a coherent text description, do not use newlines
- Use English
- age must be integer 30, gender must be string "other"
- Account speech must fit its identity and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        # Helper function for real-time saving
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # Real-time write to file (even for fallback)
                    save_profiles_realtime()
        
        print(f"\n{'='*60}")
        print(f"Persona generation complete! Total {len([p for p in profiles if p])} Agents generated")
        print(f"{'='*60}\n")
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profiles as CSV format (consistent with OASIS official requirements)
        
        OASIS Twitter CSV fields:
        - user_id: User ID (sequential starting from 0 based on CSV order)
        - user_name: Username in the system
        - name: User real name
        - bio: Short public bio (displayed on user profile page)
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Consistent with to_reddit_format(), ensuring OASIS can read it correctly.

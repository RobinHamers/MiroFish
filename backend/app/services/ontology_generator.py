...
Your 10 entity types must contain the following layers:

A. **Fallback types (required, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual. When a person does not belong to any more specific person type, assign them here.
   - `Organization`: Fallback type for any organization. When an organization does not belong to any more specific organization type, assign it here.
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # Truncate if text exceeds 50K characters (only affects content sent to LLM, not graph building)

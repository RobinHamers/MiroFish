...
Dynamically updates Agent activities from simulation into Zep graph.
"""
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        ...
        # Returns "agent name: activity description" format
    ...
    def _describe_repost(self) -> str:
        """Repost a post - includes original content and author information"""
    ...
    def _describe_follow(self) -> str:
        """Follow a user - includes followed user's name"""
    ...
    def _describe_create_comment(self) -> str:
        """Post a comment - includes comment content and information of the post commented on"""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    Monitors simulation action logs and updates new agent activities into Zep graph in real-time.
    Groups by platform and sends to Zep in batches of BATCH_SIZE.
    """
    
    # Batch size for sending activities
    BATCH_SIZE = 5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Activity buffers grouped by platform
        ...
        # Statistics
        self._total_activities = 0  # Count of activities added to queue
        self._total_sent = 0        # Count of batches successfully sent to Zep
        self._total_items_sent = 0  # Count of activities successfully sent to Zep
        self._failed_count = 0      # Count of failed batch sends
        self._skipped_count = 0     # Count of filtered activities (DO_NOTHING)
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        All meaningful behaviors are added, including:
        - CREATE_POST, CREATE_COMMENT, QUOTE_POST, SEARCH_POSTS, SEARCH_USER,
        - LIKE_POST/DISLIKE_POST, REPOST, FOLLOW, MUTE, LIKE_COMMENT/DISLIKE_COMMENT
        """
    ...
    def _worker_loop(self):
        """Background worker loop - batches and sends activities to Zep by platform"""
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """
        Sends a batch of activities to Zep graph (merged into one text).
    ...
    def _flush_remaining(self):
        """Sends remaining activities in queue and buffers"""
        # Process remaining activities in queue
        ...
        # Send remaining activities in buffers for each platform
        with self._buffer_lock:
...
class ZepGraphMemoryManager:
    """
    Manages multiple simulation's Zep Graph Memory Updaters.
    ...
    def create_updater(cls, simulation_id: str, graph_id: str) -> ZepGraphMemoryUpdater:
        """
        Creates a graph memory updater for a simulation.

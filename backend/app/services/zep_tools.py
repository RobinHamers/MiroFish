...
2. PanoramaSearch (Wide Search) - Obtains a complete overview, including expired content.
...
class SearchResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [f"Search Query: {self.query}", f"Found {self.total_count} items"]
...
class PanoramaResult:
    ...
    # Expired/invalid facts (history records)
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            f"## Wide Search Result (Future Panorama View)",
            f"Query: {self.query}",
            f"\n### [Statistics]",
...
class InterviewResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            ...
            "\n### [Selection Reasons for Interviewees]",
...
class ZepToolsService:
    ...
    [Core Retrieval Tools - Optimized]
    1. insight_forge - Deep Insight Retrieval (Most powerful, generates sub-questions, multi-dimensional retrieval)
    2. panorama_search - Wide Search (Obtains a complete overview, including expired content)
    3. quick_search - Simple Search (Fast retrieval)
    4. interview_agents - Deep Interview (Interviews simulated Agents to get multi-perspective views)
    
    [Basic Tools]
    - search_graph - Semantic search on graph
    - get_all_nodes - Get all nodes of graph
    - get_all_edges - Get all edges of graph (including temporal info)
    - get_node_detail - Get detailed info of a node
    ...
    def __init__(self, api_key: Optional[str] = None, llm_client: Optional[LLMClient] = None):
        ...
        # LLM client for generating sub-questions in InsightForge
    ...
    def search_graph(
        ...
        try:
            search_results = self._call_with_retry(
                ...
                operation_name=f"search_graph(graph={graph_id})"
            )
    ...
    def _local_search(
        ...
        def match_score(text: str) -> int:
            """Calculate matching score for text against query"""
            if not text:
                return 0
    ...
    def insight_forge(
        ...
        The most powerful hybrid retrieval function. Decomposes the question and retrieves multi-dimensionally:
        ...
        # Also search for the original main query
        ...
        # Step 3: Extract relevant entity UUIDs from edges
        entity_uuids = set()
        ...
        # Get details of all relevant entities
        entity_insights = []
        node_map = {}
        ...
        # Step 4: Construct all relationship chains
        relationship_chains = []
        for edge_data in all_edges:
            if isinstance(edge_data, dict):
                source_uuid = edge_data.get('source_node_uuid', '')
    ...
    def _generate_sub_queries(
        ...
        """Use LLM to decompose a complex question into multiple sub-questions."""
        system_prompt = """You are an expert in question analysis. Your task is to decompose a complex question into multiple sub-questions that can be independently observed in a simulated world.
...
Return sub-questions list in JSON format."""
    ...
    def panorama_search(
        ...
        Obtains a complete overview, including all relevant content and historical/expired information.
        """
    ...
    def quick_search(
        ...
        """[QuickSearch - Simple Search] Fast, lightweight retrieval tool."""
    ...
    def interview_agents(
        ...
        Calls real OASIS interview API to interview running Agents in simulation.
        """
        ...
        if not profiles:
            ...
            result.summary = "No interviewee persona files found"
            return result
        ...
        # Optimization prefix for Agent response format
        INTERVIEW_PROMPT_PREFIX = (
            "You are participating in an interview. Please answer the following questions directly in plain text, "
            "incorporating your persona and all past memories and actions.\n"
            "Requirements:\n"
            ...
            "4. Answer each question one by one, prefixed with 'Question X:' (X is the question number).\n"
            "5. Separate answers to different questions with an empty line.\n"
        ...
        try:
            ...
            if not api_result.get("success", False):
                ...
                result.summary = f"Interview API call failed: {error_msg}. Please check simulation status."
                return result
            ...
            result.summary = f"Interview failed: {str(e)}. Please ensure simulation is running."
            return result
        except Exception as e:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Cleans JSON tool call wraps in Agent replies to extract actual content."""
    ...
    def _select_agents_for_interview(
        ...
        """Select Agents most suitable for interview using LLM."""
        agent_summaries = []
        ...
        system_prompt = """You are a professional interview planner. Your task is to select the most suitable subjects for interview based on interview requirements.

Criteria:
1. Agent identity/profession is relevant to topic.
2. Agent might hold unique or valuable views.
3. Select diverse perspectives (Supporters, Opponents, Neutrals, Experts).
...
Select at most {max_agents} Agents and explain why."""
...
2. Different roles might have different answers.
3. Cover facts, views, and feelings.
4. Natural language.
5. Concise (within 50 words).
6. Direct questions without prefixes.
...
- Do not use Markdown headers or separators.
- Use quotes for interviewee's original words.
- Use **bold** for key terms, but no other Markdown."""
...
Generate summary."""
        ...
        try:
            ...
            return f"Interviewed {len(interviews)} subjects: " + ", ".join([i.agent_name for i in interviews])

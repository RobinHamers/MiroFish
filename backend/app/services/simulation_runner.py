...
Runs simulations in the background and records actions for each Agent, supporting real-time status monitoring.
"""
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Optional max rounds to truncate simulation
        enable_graph_memory_update: bool = False,  # Whether to update activity to Zep graph
        graph_id: str = None  # Required if enable_graph_memory_update is True
    ) -> SimulationRunState:
        """
        Start simulation
        ...
        Args:
            ...
            max_rounds: Optional max rounds
        ...
        if not os.path.exists(config_path):
            raise ValueError(f"Simulation config does not exist, please call /prepare interface first")
        ...
        if enable_graph_memory_update:
            if not graph_id:
                raise ValueError("graph_id must be provided when enable_graph_memory_update is True")
        ...
        try:
            ...
            # Ensure UTF-8 encoding for subprocess, especially on Windows
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'
    ...
    def get_all_actions(
        ...
        """Get complete action history for all platforms"""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        actions = []
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """Clean up simulation run logs (to force restart)"""
        import shutil
    ...
    def register_cleanup(cls):
        """Register cleanup functions on application startup"""
    ...
    def close_simulation_env(
        ...
        """Close simulation environment (graceful exit from command mode)"""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            raise ValueError(f"Simulation does not exist: {simulation_id}")
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Close environment command sent (waiting for response timed out, environment probably closing)"
            }
    ...
    def get_interview_history(
        ...
        """Get Interview history records from databases"""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)

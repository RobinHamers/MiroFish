...
# Adding this prefix can prevent Agents from calling tools and respond directly with text
INTERVIEW_PROMPT_PREFIX = "Based on your persona, all past memories and actions, please reply to me in text directly without calling any tools: "


def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize Interview questions by adding a prefix to prevent Agents from calling tools
...
def create_simulation():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Project has not yet built a graph, please call /api/graph/build first"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) remain in backend/scripts/ directory and are no longer copied to simulation directories
    ...
    # List of necessary files (excluding scripts, which are in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # All of the following statuses indicate preparation work is complete:
        # - ready: Prepared, ready to run
        # - preparing: If config_generated=True, it means it's finished
...
def prepare_simulation():
    ...
    Features:
    - Auto-detection of completed preparation work to avoid duplicate generation
    - Directly returns existing results if already prepared
    ...
    try:
        ...
        # This allows frontend to get the expected total number of Agents immediately after calling prepare
        try:
            logger.info(f"Synchronously get entity count: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # Quickly read entities (no edge info needed, just for counting)
            filtered_preview = reader.filter_defined_entities(
                graph_id=state.graph_id,
                defined_entity_types=entity_types_list,
                enrich_with_edges=False  # No edge info for speed
            )
            # Save entity count to state (available immediately for frontend)
            ...
            # Failure here doesn't affect subsequent workflow; background task will re-retrieve
...
def get_prepare_status():
    ...
    2. Check for existing completed preparation via simulation_id
    
    Request (JSON):
        {
            "task_id": "task_xxxx",          // Optional, task_id returned by prepare
            "simulation_id": "sim_xxxx"      // Optional, simulation ID (for checking existing prep)
        }
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    returning the latest one (sorted by created_at) if multiple exist.
...
def get_simulation_history():
    ...
    Used for historical project display on the homepage, returns simulations with
    rich information such as project names and descriptions.
            ...
            "data": [
                {
                    ...
                    "simulation_requirement": "If a university releases...",
                    "status": "completed",
                    "entities_count": 68,
                    "profiles_count": 68,
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use total_rounds set by user, fallback to recommended rounds if not set
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get Agent Profiles in real-time (used to view progress during generation)
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns extra metadata (file modification time, whether generating, etc.)
    - Can return partial information even before full configuration is generated
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        - time_config: time settings (duration, rounds, peak/off-peak slots)
        - agent_configs: activity settings for each Agent (activity level, frequency, stance, etc.)
        - event_config: event settings (initial posts, hot topics)
        - platform_configs: platform-specific settings
        - generation_reasoning: reasoning provided by LLM for the configuration
    """
    try:
        manager = SimulationManager()
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "enable_graph_memory_update": false,   // Optional: whether to update Zep graph memory with Agent activities
            "force": false                         // Optional: force restart (stops ongoing simulation and clears logs)
        }

    About the force parameter:
        - When enabled, if simulation is running or completed, it will stop and clear run logs first
        - Cleared content: run_state.json, actions.jsonl, simulation.log, etc.
        - Configuration file (simulation_config.json) and profile files are not cleared
        - Suitable for scenarios requiring a full re-run

    About enable_graph_memory_update:
        - When enabled, all Agent activities (posts, comments, likes, etc.) are updated to Zep graph memory in real-time
        - Allows the graph to "remember" the simulation process for subsequent analysis or AI dialogue
        - Requires a valid graph_id associated with the simulation project
    ...
    try:
        ...
        # Intelligent status handling: if preparation is complete, allow restart
        if state.status != SimulationStatus.READY:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"Simulation is running, please call /stop first, or use force=true to restart"
                            }), 400

                # Clear run logs if in force mode
                if force:
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "enable_graph_memory_update requires a valid graph_id, please ensure the project has a built graph"
                }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # Get actions of the current round (recent_actions only shows latest round)
...
def get_agent_stats(simulation_id: str):
    ...
    Used by frontend to display Agent activity rankings, action distribution, etc.
    """
    try:
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (entering wait mode after completing simulation loop)
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // Optional, specify platform (twitter/reddit)
                                               // If not specified: interview Agent on both platforms simultaneously in dual-platform simulation
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or closed. Please ensure simulation is complete and in wait mode."
            }), 400
        
        # Optimize prompt to prevent Agents from calling tools
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",              // Optional, default platform (overridden by individual settings)
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            # Validate individual platform setting
        ...
        # Optimize prompt for each interview item to prevent Agents from calling tools
...
def interview_all_agents():
    """
    Global Interview - Ask the same question to all Agents
...
def get_interview_history():
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",          // Optional, platform type (reddit/twitter)
                                           // Returns history for both platforms if not specified
    ...
    try:
        ...
        simulation_id = data.get('simulation_id')
        platform = data.get('platform')  # Returns both if not specified
        agent_id = data.get('agent_id')
...
def get_env_status():
    ...
    Check if simulation environment is alive (capable of receiving Interview commands)
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Environment is running, capable of receiving Interview commands"
            }
        }
    """
    try:
...
def close_simulation_env():
    ...
    Sends a close command to the simulation, allowing it to gracefully exit wait mode.
    
    Note: This is different from the /stop interface, which forcefully terminates the process.
    This interface allows the simulation to close gracefully.

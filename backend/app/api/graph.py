...
Adopts a project context mechanism, server-side state persistence.
"""
...
def reset_project(project_id: str):
    """
    Reset project status (used for re-building the graph)
    """
    project = ProjectManager.get_project(project_id)
...
def generate_ontology():
    ...
    Parameters:
        ...
        simulation_requirement: Description of simulation requirements (required)
        project_name: Project name (optional)
        additional_context: Extra instructions (optional)
    ...
    try:
        ...
        if not document_texts:
            ...
            return jsonify({
                "success": False,
                "error": "Failed to process any documents, please check file formats"
            }), 400
        ...
        # Generate ontology
        logger.info("Call LLM to generate ontology definition...")
    ...
    Returns:
        {
            ...
            "data": {
                "project_id": "proj_xxxx",
                "task_id": "task_xxxx",
                "message": "Graph building task has started"
            }
        }
    """
    try:
        ...
        force = data.get('force', False)  # Force re-build
        
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": "Project has not yet generated an ontology, please call /ontology/generate first"
            }), 400
        
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is currently being built, please do not resubmit. To force re-build, please add force: true",
                "task_id": project.graph_build_task_id
            }), 400
        
        # If force re-build, reset status
        ...
        def build_task():
            ...
            try:
                ...
                # Wait for Zep processing (check processed status of each episode)
                task_manager.update_task(
                    task_id,

...
- Environment remains active after simulation completion, entering wait-for-command mode
...
if sys.platform == 'win32':
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        # Set default encoding only for text mode (non-binary) and when encoding is not specified
...
# Add filter immediately upon module load to ensure it takes effect before camel code execution
logging.getLogger().addFilter(MaxTokensWarningFilter())


def disable_oasis_logging():
    """
    Disable detailed logging from the OASIS library
...
# Twitter available actions (excludes INTERVIEW, which is manually triggered via ManualAction)
TWITTER_ACTIONS = [
...
# Reddit available actions (excludes INTERVIEW, which is manually triggered via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        # Parallel interview on both platforms
        tasks = []
    ...
    def _get_interview_result(self, agent_id: int, platform: str) -> Dict[str, Any]:
        """Fetch latest Interview result from database"""
...
# Non-core action types to filter out (these actions have lower value for analysis)
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Last read maximum rowid value (use rowid instead of created_at because formats vary across platforms)
...
def _enrich_action_context(
    ...
    try:
        ...
        # Repost: supplement original post content and author
            ...
            if new_post_id:
                # original_post_id of a repost points to the original post
                cursor.execute("""
        ...
        # Quote post: supplement original post content, author, and quote content
            ...
            # Get quote content of the quote post
            if new_post_id:
                cursor.execute("""
        ...
        # Create comment: supplement info of the commented post
...
def _get_post_info(
    ...
    try:
        ...
        if row:
            ...
            # Prioritize using name from agent_names
            author_name = ''
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If boost LLM is configured, different platforms can use different API providers during parallel simulation to increase concurrency.
    ...
    # Select which LLM to use based on parameters and config status
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("Missing API Key configuration, please set LLM_API_KEY in the .env file in project root")
...
async def run_twitter_simulation(
    ...
    # If an agent is not in the configuration, use OASIS's default name
    ...
    total_actions = 0
    last_rowid = 0  # Track the last processed row ID in database (use rowid to avoid created_at format issues)
    ...
    # Truncate if maximum rounds is specified
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Record round end even when no agents are active (actions_count=0)
            if action_logger:
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM configuration (if available, else fallback to general config)
...
async def main():
    ...
    # Create shutdown event at the start of main to ensure the whole program responds to exit signals
    ...
    if args.max_rounds:
        ...
        if args.max_rounds < config_total_rounds:
            log_manager.info(f"  - Actual rounds execution: {args.max_rounds} (truncated)")
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: simulation does not exit upon completion, but waits for interview commands
    When a termination signal is received, we need to:
    1. Notify the asyncio loop to exit the wait
    2. Allow the program to clean up resources normally (close databases, environments, etc.)
    3. Then exit
    """
